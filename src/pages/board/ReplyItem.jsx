  ...
  return (
    <div className="shadow-md rounded-sm p-4 mb-4 bg-gray-50 dark:bg-gray-600">
      <div className="flex justify-between items-center mb-2">
        ...
        <time className="ml-auto text-gray-500 dark:text-gray-400" dateTime={ item.updatedAt }>{ item.updatedAt }</time>
          ...
          <>
            ...
            { user?._id === item.user._id && (
              <div className="ml-auto flex whitespace-nowrap">
                <Button bgColor="gray" size="sm" onClick={ () => setEditMode(true) }>수정</Button>
                <Button bgColor="red" size="sm" onClick={ () => handleDelete(item._id) }>삭제</Button>
              </div>
            ) }
          </>
        ) }
